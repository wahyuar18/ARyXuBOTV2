            ...
            case 'bitly':
               ...
               client.updatePresence(from, Presence.composing) 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					break
                case 'blowjob':
                if (isGroup) return  reply( 'Buset sangean:v, ni fitur bukan buat gc ye anjim')
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
          ...
          if (!isRegistered) return reply(ind.noregis())
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'add':
					...
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args[0] === 'buka') {
					    reply(`*𝑺𝑼𝑪𝑪𝑬𝑺 _𝐎𝐏𝐄𝐍_ 𝑮𝑹𝑼𝑶𝑷*`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`*𝑺𝑼𝑪𝑪𝑬𝑺 𝑪𝑳𝑶𝑺𝑬 𝑮𝑹𝑼𝑶𝑷*`)
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Hadehhh beban beban,𝗼𝘁𝘄 𝗸𝗶𝗰𝗸 beban 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Hadehhh beban beban,𝗼𝘁𝘄 𝗸𝗶𝗰𝗸 beban @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
